...
``aiida-ase3`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-ase3`` repository (preferably using ``aiida-ase3`` as the project name)

The documentation is now available at `aiida-ase3.readthedocs.io <http://aiida-ase3.readthedocs.io/>`_.
...
.. note::
   ...
   When updating the plugin package to a new version, remember to update the version number both in ``setup.json`` and ``aiida_ase3/__init__.py``.
